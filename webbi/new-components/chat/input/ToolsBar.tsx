...
import useLocalStorageListener from '@/hooks/use-local';
import { ChatContentContext } from '@/pages/chat';
import { FormatPainterFilled, LoadingOutlined, PauseCircleFilled, HourglassFilled ,} from '@ant-design/icons';
...
const ToolsBar: React.FC<{
  ...
  useEffect(() => {
    ...
    const list = (updatedTraceInfo || []).map((item: any) => {
      return {
        label: <Tag color='#e10b14'> {item.context}</Tag>,
        value: item.context,
      };
    });
    setOpts(list || []);
  ...
  const rightToolsConfig: ToolsConfig[] = useMemo(() => {
    return [
      {
        tip: t('stop_replying'),
        icon: <PauseCircleFilled className={classNames({ 'text-[#0c75fc]': canAbort })} />,
        can_use: canAbort,
        key: 'abort',
        onClick: () => {
          if (!canAbort) {
    ...
    <div className='flex flex-col  mb-2'>
      <div className='flex items-center justify-between h-full w-full'>
        <div className='flex gap-3 text-lg'>
          <Select
            placeholder='快捷词语'
            className='w-160 h-8 rounded-3xl'
            onSelect={(val: any) => {
              setUserInput(val);
            }}
            allowClear
